...
unsigned int had_int4 = 0;
unsigned int had_int5 = 0;
...
void encoder_change(struct encoder_s *p, unsigned char val)
{
   int x1;
   int x2;
   int j;
    ...
    p->last_high     = p->high;
    p->last_low      = p->low;
    p->last_is_white = p->is_white;

    // quickly grab the hardware registers
    p->high     = Wheel_Clock;
    p->low      = TMR2;
    p->is_white = val;

   // now we can calculate the delta
   if (p->high < p->last_high)
   {
      // rollover has happened
      p->delta_high = p->high + (~(p->last_high));
   }
   else
   {
      p->delta_high = p->high - p->last_high;
   ...
   if (p->white[0] != 2)
   {
      for(j=15;j>0;j--)
      {
          p->period[j] = p->period[j-1];
          p->white [j] = p->white[j-1];
      }
   }

   p->period[0] = ((p->delta_high & 0xFFFFFF) << 8) + p->delta_low;
   p->white [0] = p->is_white;
...
{
	extern unsigned char debug_cycle;
    char buffA[32];
...
#if 0
        printf("last %ld:%d:%d\n",
               (unsigned long)p->last_high,
               (int)p->last_low,
               (int)p->last_is_white);
#endif

//printf("  current Wheel_Clock => %ld\n", (unsigned long)Wheel_Clock);
//printf("  current TMR2        => %d\n", (int)TMR2 );
#if 0
       	printf("cur  %ld:%d:%d\n",
			(unsigned long)p->high, (int)p->low, (int)p->is_white);
#endif       
 	    printf("delt %ld:%d\n",
			(unsigned long)p->delta_high, (int)p->delta_low);
#if 0
        printf("ary[] = %ld %ld %ld %ld\n",
			(long)p->period[0], (long)p->period[1],
            (long)p->period[2], (long)p->period[3]);

		printf("val[] = %d %d %d %d\n",
			(int)p->white[0], (int)p->white[1],
			(int)p->white[2], (int)p->white[3]);
#endif
        printf("period[1] => %ld\n", (long)p->period[1]);
#if 0
        printf("msec[] = %ld\n", (long)(((float)(p->period[1]) * 408.0e-6)*1.0e3) );
#endif
	}
}
...
*
*******************************************************************************/
struct encoder_s right_encoder;
struct encoder_s left_encoder;

void Int_4_Handler(unsigned char RB5_State)
{
	//Right Encoder Interrupt
    //encoder_change(&right_encoder, rc_dig_in04);
}
...
void Int_5_Handler(unsigned char RB6_State)
{
    ...
    //encoder_change(&left_encoder, rc_dig_in05);
}
