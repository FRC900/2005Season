...
printf(" 0 = > 1, down\n");
		...
		else if (auto_caster_GBL && (cycle == 0))
		{
			PUT_CASTER_DOWN;
			state = 4;
			cycle = 25;
...
#else
#define CASTER_TRIP_HIGH 134
#define CASTER_TRIP_LOW  120
	if ((p1_y > CASTER_TRIP_HIGH) && (p3_y < CASTER_TRIP_LOW))
	{
		auto_caster_GBL = 1;
	}
	else if ((p1_y < CASTER_TRIP_LOW) && (p3_y > CASTER_TRIP_HIGH))
	{
		auto_caster_GBL = 1;
	}
#endif
