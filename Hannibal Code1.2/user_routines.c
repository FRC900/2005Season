...
extern unsigned int wheel_r_count;
extern unsigned int wheel_l_count;
unsigned char p1_x_old=127;
unsigned char p1_y_old=127;
unsigned char HATCommand=0;
unsigned char casterToggle=0;
unsigned char whinchCommand=0;
unsigned char upToggle=0;
unsigned char downToggle=0;
unsigned char whinchCounter=0;
unsigned char whinchColor_old=0;
unsigned long counter=0;
unsigned char debug_cycle = 0;

unsigned int setPointL_old=0;
unsigned int setPointR_old=0;
unsigned int processPointL_old=0;
unsigned int processPointR_old=0;
int errorL_old=0;
int errorR_old=0;
unsigned char pwm15_old=127;
unsigned char pwm13_old=127;
unsigned char periodTable[14];
/*******************************************************************************
...
*/
	...
	if(whinchCommand=2 && whinchCounter==0)
	{
		relay1_fwd=0;
		whinchCommand=0;
	}
	else
	{
		...
		{
			printf("P2 Top\n");
			if(!p2_sw_aux2 && whinchCommand==0 && downToggle==0)
			{
				whinchCommand=2;
		...
		{
			printf("P2 Aux2\n");
			if(!p2_sw_top && whinchCommand==0 && upToggle==0)
			{
				whinchCommand=1;
				if(debug_cycle) printf("Current: %d",(int)Get_Analog_Value(rc_ana_in02));
			}
...
if(whinchCommand!=0)
	{
		...
		else
		{
			...
			{
				pwm01=254;
				if(whinchColor_old!=rc_dig_in06) 
				{
					whinchCounter--;
				}
			}
